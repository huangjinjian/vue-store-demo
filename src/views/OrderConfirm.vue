              ...
              <li v-for="item in cartList" v-if="item.checked=='1'">
                <div class="cart-tab-1">
                  <div class="cart-item-pic">
                    <img v-lazy="'images/'+item.prodcutImg" :alt="item.productName" />
                  </div>
...
export default {
  ...
  computed: {
    ...
    subTotal() {
      ...
      this.$store.state.cartList.forEach(item => {
        total = item.checked
          ? total + parseFloat(item.prodcutPrice) * parseFloat(item.productNum)
          : total;
      });
      return total;
    },
  ...
  methods: {
    async payMent() {
      let res = await this.$http.post(this.$api.Users.payment, {
