      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li
                  ...
                  v-bind:class="{ check: hoverIndex == index , check: chooseIndex == index } "
                ...
                >
                  ...
                  <div
                    class="addr-opration"
                    v-else
                    :class="{'addr-set-default':index != hoverIndex}"
                  >
                    <a href="javascript:;" class="addr-set-default-btn" @click="setDefault(item)">
                      <i>Set default {{chooseIndex}}</i>
                    </a>
                  </div>
                </li>
                <li class="addr-new" @click="addAddr">
            ...
            <div class="shipping-addr-more" v-if="limit > 3">
              <a
                class="addr-more-btn up-down-btn"
                href="javascript:;"
                @click="expand"
                v-bind:class="{ open: more }"
              >
                more
                <i class="i-up-down">
...
export default {
  ...
  computed: {
    filterList() {
      if (this.addrList.length > 3 && !this.more) {
        return this.addrList.slice(0, 3);
      } else {
    ...
    chooseIndex() {
      if (this.checkIndex >= 0) {
        ...
        this.addrList.forEach((item, index) => {
          Dindex = item.isDefault ? index : Dindex;
          if (item.isDefault) {
            this.$store.commit("updataAddrId", this.addrList[Dindex].addressId);
          }
        });
        return Dindex;
      }
  ...
  watch: {
    chooseIndex() {
      this.$store.commit(
        "updataAddrId",
        this.addrList[this.chooseIndex].addressId
      );
    }
  },
  async mounted() {
  ...
  methods: {
    ...
    async getAddresList() {
      let addrRes = await this.$http.post(this.$api.Users.address, {
        user_id: this.$store.state.userId
      });
      this.addrList = addrRes.state == 1 ? addrRes.data.addressList : [];
    },
    addAddr() {
      this.$router.push({
        path: "/address-edit?type=add"
      });
    },
